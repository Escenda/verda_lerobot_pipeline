...
def deploy_remote_payload(
  ...
  try:
    ...
    for fname in ("setup_env.sh", "train_lerobot_entry.py"):
...
def fetch_remote_logs(
  ...
  try:
    ...
    for log_name in ("setup_env_train.log", "setup_env_resume_hub.log"):
...
def main() -> None:
  ...
  try:
    ...
    try:
      deploy_remote_payload(ssh, env_file, project_root)
      ...
      status = run_remote_training(ssh, mode=mode, checkpoint_repo_id=checkpoint_repo_id)
